...
from gbgo.data.index_processor import KGSIndex
from gbgo.data.sampling import Sampler
from gbgo.data.generator import DataGenerator
from gbgo.encoders.base import get_encoder_by_name
...
# end::load_generator[]
    ...
    def process_zip(self, zip_file_name, data_file_name, game_list):
        ...
        for name in sgf_names:
            ...
            moves = self.sgf_num_moves(sgf_content)
            sgf = Sgf_game.from_string(sgf_content)
            feature_shape = np.insert(shape, 0, np.asarray([moves]))

            if features is not None and (features.shape[0] + moves > 100000):
                feature_file = self.data_dir + '/' + data_file_name + str(batch_no) + '_features'
                label_file = self.data_dir + '/' + data_file_name + str(batch_no) + '_labels'
            ...
            if features is None:
                ...
                features = np.concatenate((features, np.zeros(feature_shape)), axis=0)
            if labels is None:
                labels = np.zeros((moves,))
            else:
                labels = np.concatenate((features, np.zeros(feature_shape)), axis=0)
            ...
            for item in sgf.main_sequence_iter():
                ...
                if color is not None:
                    ...
                    if first_move_done and point is not None:
                        features[move_counter] = self.encoder.encode(game_state)
                        labels[move_counter] = self.encoder.encode_point(point)
                        move_counter += 1
    ...
    def sgf_num_moves(self, sgf_string):
